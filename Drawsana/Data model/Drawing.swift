...
public class Drawing: Codable {
    ...
    private func tryDecodingAllShapes(_ container: inout UnkeyedDecodingContainer) -> [Shape] {
        ...
        multiDecoder.tryDecoding(StampShape.self)
    ...
    public func getShape<T: Shape>(of _: T.Type, at point: CGPoint, filter: ((Shape) -> Bool)? = nil) -> T? {
        return shapes
            .compactMap { $0 as? T }
...
public class MultiDecoder<ResultType> {
    ...
    /// nothing.
    public func tryDecoding<T: Shape>(_: T.Type) {
        do {
