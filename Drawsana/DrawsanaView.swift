...
public protocol DrawsanaViewDelegate: AnyObject {
    ...
    func drawsanaView(_ drawsanaView: DrawsanaView, didChangeImageName imageName: String)
}
...
public class DrawsanaView: UIView {
    ...
    public lazy var operationStack: DrawingOperationStack = {
        DrawingOperationStack(drawing: drawing)
    }()
    ...
    public func render(over image: UIImage?) -> UIImage? {
        ...
        return DrawsanaUtilities.renderImage(size: size) { (_: CGContext) -> Void in
            image?.draw(at: .zero)
    ...
    private func reapplyLayerContents() {
        drawingContentView.layer.contents = persistentBuffer?.cgImage
    }
    ...
    private func applySelectionViewState() {
        ...
        if shape is StampShape {
            ...
            var translation = shape.transform.translation + CGPoint(x: 0, y: -selectionBounds.size.height)
...
extension DrawsanaView: DrawingDelegate {
    ...
    func drawingDidUpdateShape(_: Shape) {
        redrawAbsolutelyEverything()
        applyToolSettingsChanges()
    }
...
extension DrawsanaView: ToolSettingsDelegate {
    func toolSettings(
        _: ToolSettings,
        didSetSelectedShape _: ShapeSelectable?
    ) {
        applySelectionViewState()
    ...
    func toolSettings(
        _: ToolSettings,
        didSetIsPersistentBufferDirty _: Bool
    ) {
...
extension DrawsanaView: UserSettingsDelegate {
    ...
    func userSettings(_ userSettings: UserSettings, didChangeImageName imageName: String) {
        tool?.apply(context: toolOperationContext, userSettings: userSettings)
        applyToolSettingsChanges()
        delegate?.drawsanaView(self, didChangeImageName: imageName)
    }
}
